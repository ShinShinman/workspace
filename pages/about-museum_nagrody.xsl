...
<xsl:template match="data">
	<section class="museum">
		...
		<article>
			<h1><xsl:value-of select="//plh-page/page/page/item[@lang = //fl-languages/current-language/@handle]" /></h1>
			<ul>
				<xsl:apply-templates select="museum-awards/entry" />
			</ul>
		</article>
	</section>
</xsl:template>

<xsl:template match="museum-awards/entry">
	<xsl:choose>
