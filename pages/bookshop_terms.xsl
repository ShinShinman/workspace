...
<xsl:template match="data">
	...
	<section class="bookshop-page">
		<article>
			<xsl:apply-templates select="bookshop-terms/entry" />
		</article>
	</section>
</xsl:template>

<xsl:template match="bookshop-terms/entry">
...
<xsl:template name="language-button">
	...
	<xsl:choose>
		<xsl:when test="$lang = 'pl'">
			<a href="{$root}/en/bookshop/terms/" class="icon">E</a>
		</xsl:when>
		<xsl:otherwise>
			<a href="{$root}/pl/wydawnictwa/regulamin/" class="icon">P</a>
		</xsl:otherwise>
	</xsl:choose>
</xsl:template>
